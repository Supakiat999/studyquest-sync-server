(function installStudyQuestV20Module(root) {
  ...
  const WEEKLY_VIEW_STORAGE_SUFFIX = '_v21_weekly_ui';
  ...
  function resizeQuickNote(input) {
    if (!input) return;
    input.style.height = 'auto';
    const maxHeight = Number.parseFloat(root.getComputedStyle?.(input)?.maxHeight || '');
    const cap = Number.isFinite(maxHeight) ? maxHeight : 82;
  ...
  function renderWeeklyFeatureCard() {
    ...
    card.innerHTML = `<div class="v20-weekly-tracking-head">
      <div><div class="v20-weekly-tracking-title" id="v20WeeklySubjectTrackingTitle">Subject Track <span style="color:var(--accent2);font:800 9px 'Space Mono',monospace;">v20 local</span></div><div class="v20-weekly-tracking-copy">An additive, local-only overlay over the existing Weekly records. Notes and columns are read-only references here; edit them in By week. Choose which existing weeks to display below.</div></div>
  ...
  async function rollbackState(core, previous, label) {
    try { core.setSQState?.(previous); } catch {}
