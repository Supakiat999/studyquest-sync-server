...
- When hosted v19 finds a stale browser copy and every reviewed choice selects the Account backup, it archives and verifies the browser, IndexedDB, and pending-outbox copies before loading the Account backup on that computer. This device-only action performs no cloud POST and does not increment the account revision; mixed Smart Merges retain the existing revision/hash-protected save flow.
