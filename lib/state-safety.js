...
const STATE_MANIFEST_VERSION = 1;

function stateManifest(state) {
  const records = collectStateRecords(state);
  const contentState = state && typeof state === "object" && !Array.isArray(state) ? { ...state } : state;
  if (contentState && typeof contentState === "object" && !Array.isArray(contentState)) delete contentState.updatedAt;
  const recordHashes = {};
  const counts = {};
  Array.from(records.entries())
    .sort(([left], [right]) => left.localeCompare(right))
    .forEach(([key, record]) => {
      recordHashes[key] = stateHash(record.value);
      counts[record.collection] = Number(counts[record.collection] || 0) + 1;
    });
  const manifest = {
    version: STATE_MANIFEST_VERSION,
    counts,
    records: recordHashes,
    contentHash:stateHash(contentState),
  };
  return { ...manifest, hash:stateHash(manifest) };
}

function unapprovedManifestRemovals(currentManifest, incomingManifest, changeSet) {
  const currentRecords = currentManifest?.records && typeof currentManifest.records === "object"
    ? currentManifest.records
    : {};
  const incomingRecords = incomingManifest?.records && typeof incomingManifest.records === "object"
    ? incomingManifest.records
    : {};
  const approved = normalizeDeletionManifest(changeSet);
  const removed = Object.keys(currentRecords)
    .filter((key) => !Object.prototype.hasOwnProperty.call(incomingRecords, key))
    .map((key) => {
      const separator = key.indexOf("/");
      return {
        collection:separator === -1 ? "state" : key.slice(0, separator),
        id:separator === -1 ? key : key.slice(separator + 1),
        key,
        label:key,
      };
    });
  return { removed, unapproved:removed.filter((record) => !approved.has(record.key)) };
}

function massDeletionRisk(currentManifest, incomingManifest) {
  const currentRecords = currentManifest?.records && typeof currentManifest.records === "object"
    ? currentManifest.records
    : {};
  const incomingRecords = incomingManifest?.records && typeof incomingManifest.records === "object"
    ? incomingManifest.records
    : {};
  const currentCount = Object.keys(currentRecords).length;
  const incomingCount = Object.keys(incomingRecords).length;
  const removedCount = Object.keys(currentRecords)
    .filter((key) => !Object.prototype.hasOwnProperty.call(incomingRecords, key)).length;
  const removalRatio = currentCount > 0 ? removedCount / currentCount : 0;
  return {
    risky: removedCount >= 20 || (currentCount >= 20 && removalRatio >= 0.5),
    currentCount,
    incomingCount,
    removedCount,
    removalRatio,
...
function collectStateRecords(stateValue) {
  ...
  // v21 manual courses are account records, so they take part in deletion
  // checks, recovery exports, and conflict review like any other collection.
  const v21 = state._studyquestV21 && typeof state._studyquestV21 === "object" && !Array.isArray(state._studyquestV21)
    ? state._studyquestV21
    : {};
  const manualCourses = v21.manual && typeof v21.manual === "object" && !Array.isArray(v21.manual)
    ? v21.manual.courses
    : null;
  collectArrayRecords(records, "_studyquestV21.manual.courses", manualCourses, "manual-course");
  const courses = Array.isArray(manualCourses) ? manualCourses : [];
  courses.forEach((course, courseIndex) => {
    const courseId = recordIdentity(course, "manual-course", courseIndex);
    collectArrayRecords(records, "_studyquestV21.manual.courses.rows", course?.rows, "manual-lecture", courseId);
    collectArrayRecords(records, "_studyquestV21.manual.courses.columns", course?.columns, "manual-column", courseId);
  });

  // v22 subtasks are account records in their own namespace. Keep occurrence
  // keys and individual subtask IDs visible to deletion/conflict checks so an
  // older client cannot make a missing subtask tree look like an approved
  // deletion.
  const v22 = state._studyquestV22 && typeof state._studyquestV22 === "object" && !Array.isArray(state._studyquestV22)
    ? state._studyquestV22
    : {};
  const subtasks = v22.subtasks && typeof v22.subtasks === "object" && !Array.isArray(v22.subtasks)
    ? v22.subtasks
    : {};
  const subtaskRecords = subtasks.records && typeof subtasks.records === "object" && !Array.isArray(subtasks.records)
    ? subtasks.records
    : {};
  Object.entries(subtaskRecords).forEach(([occurrenceKey, occurrence]) => {
    const occurrenceId = String(occurrenceKey);
    records.set(`_studyquestV22.subtasks.records/${occurrenceId}`, {
      collection: "_studyquestV22.subtasks.records",
      id: occurrenceId,
      key: `_studyquestV22.subtasks.records/${occurrenceId}`,
      label: recordLabel(occurrence, occurrenceId),
      value: occurrence,
    });
    collectArrayRecords(records, "_studyquestV22.subtasks.records.subtasks", occurrence?.subtasks, "v22-subtask", occurrenceId);
  });
  collectArrayRecords(records, "_studyquestV22.subtasks.trash", subtasks.trash, "v22-trash");
  return records;
}
