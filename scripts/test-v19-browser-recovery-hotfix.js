...
// The only permitted second argument is the version this opener derived from the
// database itself, so a repair can raise the schema but nothing can lower it.
assert.ok(
  [...html.matchAll(/indexedDB\.open\(\s*DEVICE_RECOVERY_DB\s*,\s*([^)]*)\)/g)]
    .every(match => match[1].trim() === 'version'),
  'v19 must not downgrade recovery storage');
assert.match(html, /const nextVersion = db\.version \+ 1;/, 'v19 must only ever request a higher recovery schema version');
